...
<Card title="Unique Value Proposition">
  ...
  The **fractal composition** and **namespace versioning** are also unique - tc lets you zoom into a sub-topology, create a sandbox, test it, then zoom back out to the full system while maintaining graph integrity.
	...
	tc is unmatched when you have 30+ interconnected components. You can see and understand the entire system topology at a glance because relationships are explicit first-class citizens, not scattered implementation details.
</Card>
...
<Card title="What are some of the unique features ?">
- Provides 8 core entities (functions, events, routes, mutations, queues, states, channels and stores)  that are namespaced, provider-agnostic and composable as a topology.
- Separates domain-specific interfaces and code from sandbox-specific infrastructure configuration.
- Dynamically generates permissions with sane defaults and option to override.
- Provides a graphql transpiler that makes it simpler to work with Graphql subscriptions and mutations.
- Provides a ASL generator for working with complex stepfunction flows.
- Sophisticated function builders for various language runtimes
- Snapshotting, versioning, pipeline generators to work with CI-based workflows.
- Customizable orchestration between entities using transducers.
- Provides CLI, MCP and REPL interfaces
</Card>
...
:::caution
tc is still not production-ready and is being actively developed. Current version: 0.9.120
:::
