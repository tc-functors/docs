...
tc (topology composer) provides a higher-level abstraction for serverless development, focusing on the logical relationships between abstract entities (functions, events, routes, queues, channels, mutations and pages) rather than the underlying infrastructure details. It is an **application composer**.
...
:::note
- tc is not just an infrastructure deployer. It is primarily an application composer meant for developers.
