...
The late Jim Weirich gave an excellent talk on [Lisp primitives](https://www.luisdelarosa.com/the-closing-jimnote-at-mountainwest-rubyconf/) which was enlightening to me.  Lisp, the programming language created by John McCarthy in 1958, has just seven primitives (car, cdr, cons, eq, atom, cond and lambda). All computation can be expressed using these primitives. Such an elegant idea!

What if we can express the entities of a distributed computing system as a topology or graph of computation using similar primitives ?

tc is a framework that implements this idea of composing cloud primitives. tc compiles a tree of entities defined in the filesystem as a topology. This composable, namespaced, sandboxed, recursive, versioned and isomorphic topology is called a `Cloud Functor`.
...
The directory structures look identical, yet they are different - their handler logic is different. app1 and app2 are separate apps and have lifecycle of their own.
