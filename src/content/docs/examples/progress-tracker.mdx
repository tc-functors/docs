...
From the entity [definitions](/introduction/concepts#entity-abstraction), we know that the _Mutation_ entity has a unique characteric of having abstract types and subscriptions to it's change.  Per the problem statment, we need to track the status changes in an arbitrary job. Perhaps we can try to model it as a Mutation.
