...
The kneejerk reaction, as developers, is to draw an architecture diagram (or ask ChatGPT you say?) with sophisticated AWS services. With functors, we start with datastructures.
...
Let's create a sandbox and test it.
...
2. Any event entity can trigger a mutation and thus propagating that change to the subscriber, without an external database.
3. Every entity has a set of composition rules and properties. These rules magically map to constraints in the cloud (AWS et al).
