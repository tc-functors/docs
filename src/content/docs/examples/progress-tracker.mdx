...
We mentioned briefly about `resolvers`. A resolver is any backing entity (function, table etc) that returns data suitable for subscribers. For example, this is how we define `startJob`. It has a backing entity function named `starter` that takes in data with input Type (JobInput) and returns data with output Type (Job). The subscriber, HTML page in this case, gets the value of `Job` as payload.
